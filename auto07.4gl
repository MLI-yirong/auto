...
DEFINE	f_pno_bgn		CHAR(12)  -- test
DEFINE	f_pno_end		CHAR(12)  -- test
...
--LET	g_bill_stat_ym   = ARG_VAL(2)
LET	f_pno_bgn        = ARG_VAL(2)  -- test
LET	f_pno_end        = ARG_VAL(3)  -- test
...
IF	(f_rcode = FALSE)	THEN
        ...
        EXIT PROGRAM
END IF
IF	LENGTH( f_pno_bgn CLIPPED ) = 0 THEN  -- test
	LET f_pno_bgn = " "
END	IF
IF	LENGTH( f_pno_end CLIPPED ) = 0 THEN  -- test
	IF	LENGTH( f_pno_bgn CLIPPED ) = 0 THEN
		LET f_pno_end = "799999999999"
	ELSE
		LET f_pno_end = f_pno_bgn
	END	IF
END	IF
...
let	f_time = current
display	'=trace=begin:  ', f_time, ' of ', g_automatic_date, '=', f_pno_bgn, '=', f_pno_end
...
DECLARE	polf_ptr	CURSOR	WITH HOLD	FOR
	SELECT	*
        ...
        AND policy_no[1,1] = "7"
        AND insurance_type = "U"
	AND policy_no BETWEEN f_pno_bgn AND f_pno_end  -- test
		ORDER BY policy_no
